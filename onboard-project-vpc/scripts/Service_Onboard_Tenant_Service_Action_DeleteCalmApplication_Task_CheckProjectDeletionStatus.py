...
while(count < 5):
  ...
  if resp.ok:
    ...
    if project_json['metadata']['length'] == 0:
      print "Project deletion was successful", json.dumps(json.loads(resp.content), indent=4)
      exit(0)  
    else:
      print "Project Deletion deployment not ready.  Sleeping for 60 seconds"
      count = count + 1
      sleep(60) #
  # If the call failed
  else:
    print "Project deletion failed", json.dumps(json.loads(resp.content), indent=4)
    exit(1)
